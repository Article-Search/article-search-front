...
import {Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Tooltip, Popover , PopoverContent ,PopoverTrigger, Button, Input, Modal, useDisclosure, ModalFooter, ModalBody, ModalContent, ModalHeader, user} from "@nextui-org/react";
...
export default function TableUsers({columns}: {columns: ColumnType[]}) {
    ...
    useEffect(() => {
        //TODO : decomment this code after
        // const getUsers = async () => {
        //     const response = await fetch("URL");//TODO : add the url of the api
        //     if (!response.ok) {
    ...
    useEffect(() => {
        const initialUserDetails = users.reduce((acc, user) => {
            acc[user.id] = user;
            return acc;
        ...
        return (
            ...
            <Popover placement="bottom">
                ...
                <PopoverContent className="p-5">
                    {/*render a button to say are you sure and one for cancel */}
                    <p className=" font-bold text-gray-500">
